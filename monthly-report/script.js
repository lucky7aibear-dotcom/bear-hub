document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedKey) {
        ...
        document.getElementById('api-key-status').innerText = '✓ 已載入設定';
    }
...
function generateNewsletterHTML() {
        ...
        if (catName || tasks.length > 0) {
            tasksHtml += `
                <div style="margin-bottom: 20px;">
                    <h3 style="color: #4f46e5; font-size: 16px; margin-bottom: 10px; border-left: 4px solid #6366f1; padding-left: 8px;">${catName || 'Uncategorized'}</h3>
                    <ul style="list-style-type: none; padding-left: 5px;">
                        ${tasks.map(t => `
                            <li style="padding: 6px 0; border-bottom: 1px solid #f1f5f9; display: flex; align-items: baseline;">
    ...
    if (validMetrics.length > 0) {
        metricsHtml = `
            <div style="margin-bottom: 25px;">
                <h2 style="font-size: 18px; color: #1e293b; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px; margin-bottom: 15px;">Key Metrics</h2>
                <div style="display: flex; flex-wrap: wrap; gap: 15px;">
                    ${validMetrics.map(item => {
                        const label = item.querySelector('.metric-label').value;
                        const val = item.querySelector('.metric-value').value;
                        return `
                            <div style="background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0; text-align: center; min-width: 120px; flex: 1;">
                                <div style="font-size: 24px; font-weight: 600; color: #4f46e5;">${val}</div>
    ...
    if (plans.length > 0) {
        plansHtml = `
            <div style="margin-bottom: 25px;">
                <h2 style="font-size: 18px; color: #1e293b; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px; margin-bottom: 15px;">Next Month's Plan</h2>
                <ul style="list-style-type: none; padding-left: 5px;">
                    ${plans.map(p => `
                        <li style="padding: 6px 0; border-bottom: 1px solid #f1f5f9; display: flex; align-items: baseline;">
    ...
    if (fun.trim() !== '') {
        funHtml = `
            <div style="margin-bottom: 25px;">
                <h2 style="font-size: 18px; color: #1e293b; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px; margin-bottom: 15px;">Fun Findings</h2>
                <div style="background: #fffbeb; padding: 15px; border-radius: 8px; border: 1px solid #fef3c7; color: #b45309;">
                    ${fun.replace(/\n/g, '<br>')}
                </div>
            </div>
        `;
    ...
    return `
        <div style="font-family: 'Outfit', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background: white; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); border-radius: 8px; overflow: hidden;">
            <!-- Header -->
            <div style="background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%); color: white; padding: 30px; text-align: center;">
                <img src="logo.png" alt="Logo" style="max-height: 60px; margin-bottom: 15px;">
                <h1 style="font-size: 26px; margin-bottom: 10px; color: white; font-weight: 600;">${month} Report</h1>
                <p style="opacity: 0.9; font-size: 14px;">Here we record our efforts and achievements this month.</p>
            </div>
            ...
            <div style="padding: 30px;">
                ...
                <!-- Tasks -->
                <div style="margin-bottom: 25px;">
                    <h2 style="font-size: 18px; color: #1e293b; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px; margin-bottom: 15px;">Accomplished Tasks</h2>
                    ${tasksHtml}
                </div>
...
function saveApiKey() {
    ...
    document.getElementById('api-key-status').innerText = '✓ 已儲存！';
    setTimeout(() => {
        document.getElementById('api-key-status').innerText = '✓ 已載入設定';
    }, 2000);
}
...
async function handleAIExpand(inputElement, type, buttonElement) {
    ...
    if (!apiKey) {
        alert('請先在上方「API 設定」中輸入並儲存您的 Gemini API Key！');
        return;
    }
